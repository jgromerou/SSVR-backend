...
export const getMyCompany = catchAsync(async (req: Request, res: Response) => {
    ...
    if (!req.currentUser?.company_id) {
        return res.status(400).json({ status: 'error', statusCode: 400, message: 'No tienes una empresa activa asignada' });
    }

    const company = await companiesService.getMyCompany(req.currentUser?.company_id);
